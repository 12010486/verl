...
class McoreToHFWeightConverterDense(McoreToHFWeightConverterBase):
    def _convert_attention_param(self, name: str, params: list[torch.Tensor], torch_compile_stub: str = "") -> tuple[list[str], list[torch.Tensor]]:
        ...
        if "self_attention.linear_qkv.bias" in name or "self_attention.linear_qkv.weight" in name:
            ...
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.q_proj.{param_type}")
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.k_proj.{param_type}")
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.v_proj.{param_type}")
            assert len(params) == 3
        elif "self_attention.linear_proj.weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.o_proj.weight")
            assert len(params) == 1
        elif "self_attention.linear_qkv.layer_norm_weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}input_layernorm.weight")
            assert len(params) == 1
        elif "self_attention.q_layernorm.weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.q_norm.weight")
            assert len(params) == 1
        elif "self_attention.k_layernorm.weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}self_attn.k_norm.weight")
            assert len(params) == 1
        else:
            raise NotImplementedError(f"Unsupported parameter name: {name}")
        return convert_names, params

    def _convert_mlp_param(self, name: str, params: list[torch.Tensor], torch_compile_stub: str = "") -> tuple[list[str], list[torch.Tensor]]:
        ...
        if "mlp.linear_fc1.weight" in name:
            ...
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}mlp.gate_proj.weight")
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}mlp.up_proj.weight")
            assert len(params) == 2
        elif "mlp.linear_fc1.layer_norm_weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}post_attention_layernorm.weight")
            assert len(params) == 1
        elif "mlp.linear_fc2.weight" in name:
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}mlp.down_proj.weight")
            assert len(params) == 1
        else:
            raise NotImplementedError(f"Unsupported parameter name: {name}")
        return convert_names, params
    ...
    def convert_param(self, name: str, params_one_group: list[torch.Tensor]) -> tuple[list[str], list[torch.Tensor]]:
        # MLM --> name --> convert_names --> vLLM
        # MLM --> params_one_group --> vLLM

        # currently we're running vLLM with torch.compile, we need to add the "_orig_mod" accordingly
        torch_compile_stub = "_orig_mod." if is_hpu_available else ""

        if is_hpu_available:
            # When MLM is using torch.compile (on the whole model),
            # there'll be "_orig_mod" prefix, need to remove it.
            if name.startswith(torch_compile_stub):
                name = name[len(torch_compile_stub):]

        direct_name_mapping = {
            "embedding.word_embeddings.weight": f"model.embed_tokens.{torch_compile_stub}weight",
            "decoder.final_layernorm.weight": f"model.norm.{torch_compile_stub}weight",
            "output_layer.weight": f"lm_head.{torch_compile_stub}weight",
        }
        if name in direct_name_mapping:
            return [direct_name_mapping[name]], [params_one_group[0]]

        if "self_attention" in name:
            return self._convert_attention_param(name, params_one_group, torch_compile_stub=torch_compile_stub)
        elif "mlp" in name and not "pre_mlp_layernorm.weight" in name:
            return self._convert_mlp_param(name, params_one_group, torch_compile_stub=torch_compile_stub)
        elif "input_layernorm.weight" in name:
            convert_names = []
            layer_number = name.split(".")[2]
            convert_names.append(f"model.layers.{layer_number}.{torch_compile_stub}input_layernorm.weight")
            return convert_names, params_one_group
